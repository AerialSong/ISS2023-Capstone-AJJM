...
def monitor():
   ...
   file = open('s3Files/NetTest.txt', 'a')
   file.write(datetime_now + '- Monitoring started\n')   
   file.close()

   print(datetime_now + ' - Monitoring started')
   ...
   while True:         
      ...
      file.write(f"\nNumber {packet_num} | Date and time: {datetime_now}")
      file.write(f"Ethernet Frame {packet_num} - Dst MAC: {dst_mac}, Src MAC: {src_mac}, Ethernet Protocol ID: {L2_proto}")
      packet_num += 1
      ...
      if L2_proto == 8:
         ...
         file.write(f"IPv4 Packet - Src IP: {src_IP}, Dst IP: {dst_IP}, IP Protocol ID: {L3_proto}, TTL: {time_to_live}")
         ...
         if L3_proto == 1:
            ...
            file.write(f"ICMP Packet - Type: {icmp_type}, Checksum: {checksum} \nData: ")
            file.write(line_format(format_tab, data))
         ...
         # IP Protocol ID 6 is TCP
            ...
            print(format_star2, "Data:\n", line_format(format_tab, data))
            file.write(f"TCP Segment - Src Port: {src_port}, Dst Port: {dst_port}, Seq: {seq}, Ack: {ack} \nData: ")
            file.write(line_format(format_tab, data))
         ...
         # IP Protocol ID 17 is UDP
            ...
            print(format_star2, "Data:\n", line_format(format_tab, data))
            file.write(f"UDP Segment - Src Port: {src_port}, Dst Port: {dst_port}, Size: {size} \nData: ")
            file.write(line_format(format_tab, data))
         # Other IP Protocols
         else:
